...
public final class TWRecipeProvider extends ExtendedRecipeProvider {
    ...
    protected void buildRecipes() {
        masterSmithing(output, new TWMasterSmithingRecipe(Ingredient.of(TWItems.SILVER_SWORD), TWItems.MASTERFUL_SILVER_SWORD.toStack(), 20));
        masterSmithing(output, new TWMasterSmithingRecipe(Ingredient.of(TWItems.STEEL_SWORD), TWItems.MASTERFUL_STEEL_SWORD.toStack(), 20));
    ...
    public void masterSmithing(RecipeOutput output, TWMasterSmithingRecipe recipe) {
        masterSmithing(output, RecipeBuilder.getDefaultRecipeId(recipe.output.getItem()), recipe);
    }

    public void masterSmithing(RecipeOutput output, ResourceLocation location, TWMasterSmithingRecipe recipe) {
        var key = ResourceKey.create(Registries.RECIPE, location.withPrefix("master_smithing/"));
        var advancement = output.advancement()
                .addCriterion("has_the_recipe", RecipeUnlockedTrigger.unlocked(key))
                .rewards(AdvancementRewards.Builder.recipe(key))
                .requirements(AdvancementRequirements.Strategy.OR);

        if (recipe.getIngredient().isSimple()) {
            assert recipe.getIngredient().getValues().size() == 1;
            var inputItem = recipe.getIngredient().getValues().get(0).value();
            advancement.addCriterion(getHasName(inputItem), has(inputItem));
        } else {
            throw new UnsupportedOperationException();
        }

        output.accept(key, recipe, advancement.build(location.withPrefix("recipes/master_smithing/")));
    }
